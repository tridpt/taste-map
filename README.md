...
- Nút `Random gần tôi` khám phá quán mới quanh bạn từ bản đồ (OpenStreetMap/Overpass) kèm nút lưu nhanh; nếu offline thì gợi ý ngẫu nhiên trong danh sách đã lưu.
...
- Lưu ảnh quán trực tiếp trong trình duyệt; ảnh được lưu riêng trong IndexedDB (chỉ giữ id/album trong `localStorage` để tránh đầy) và phân loại theo album (món, không gian, menu, khác); panel chi tiết hiển thị ảnh theo từng album.
...
- Ảnh quán lưu trong IndexedDB; `localStorage` chỉ giữ dữ liệu quán dạng văn bản nên khó đầy, và có cảnh báo khi dung lượng `localStorage` gần đầy.
