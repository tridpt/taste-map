...
- Lập lịch trình nhiều quán: thêm quán vào lịch trình, sắp xếp thứ tự, xem tuyến nối trên bản đồ và mở chỉ đường đa điểm trên Google Maps.
...
Playwright test kiểm tra app render, import Google Maps, backup mã hóa, undo, đổi giao diện sáng/tối, lọc đang mở cửa và PWA manifest. `npm run check` cũng kiểm tra UTF-8 để tránh lưu nhầm mojibake.
